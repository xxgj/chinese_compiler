...
func TakeChineseNumberFromString(chTextString string) string {

	CHNumberStringList := []string{}
	convertedString := traditionalTextConvertFunc(chTextString, true)
	...
	if len(CHNumberStringListTemp) > 0 {
		for i := 0; i < len(CHNumberStringListTemp); i++ {
			tempCHToDigitsResult = ChineseToDigits(CHNumberStringListTemp[i], true)
...
func TakeNumberFromString(chTextString string) interface{} {
	finalResult := TakeChineseNumberFromString(chTextString)
	intNum, err := strconv.Atoi(finalResult)
	//fmt.Println(intNum)
	if err == nil {
		return intNum
	}
	floatNum, err := strconv.ParseFloat(finalResult, 64)
	if err == nil {
		return floatNum
	}
	return finalResult
